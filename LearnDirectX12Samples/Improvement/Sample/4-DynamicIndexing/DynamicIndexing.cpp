...
const float DynamicIndexing::CitySpacingInterval = 16.0f;

DynamicIndexing::DynamicIndexing(UINT width, UINT height, std::wstring name)
...
void DynamicIndexing::LoadPipeline() {
  ...
  // Create descriptor heaps.
  {
    ...
    cbvSrvHeapDesc.NumDescriptors = FrameCount * CityRowCount * CityColumnCount +  // FrameCount frames * CityRowCount * CityColumnCount.
...
void DynamicIndexing::LoadAssets() {
  ...
  // Create the root signature.
  {
    ...
    ranges[0].Init(D3D12_DESCRIPTOR_RANGE_TYPE_SRV, 1 + CityMaterialCount, 0, 0,
                   D3D12_DESCRIPTOR_RANGE_FLAG_DATA_STATIC);  // Diffuse texture + array of materials.
  ...
  // Create the pipeline state, which includes loading shaders.
  {
    ...
    std::wstring vertexShaderPath = GetAssetFullPath(L"shader_mesh_simple_vert.hlsl");
    auto hr = D3DCompileFromFile(vertexShaderPath.c_str(), nullptr, nullptr, "VSMain", "vs_5_0", compileFlags, 0, &vertexShader, &errorBlob);
    if (FAILED(hr)) {
      OutputDebugStringA((char*)errorBlob->GetBufferPointer());
      ThrowIfFailed(hr);
    }

    std::wstring pixelShaderPath = GetAssetFullPath(L"shader_mesh_dynamic_indexing_pixel.hlsl");
    hr = D3DCompileFromFile(pixelShaderPath.c_str(), nullptr, nullptr, "PSMain", "ps_5_1", compileFlags, 0, &pixelShader, &errorBlob);
  ...
  auto meshData = ReadDataFromFile(GetAssetFullPath(SampleAssets::DataFileName));

  UINT8* pMeshData = meshData.data();
  UINT meshDataLength = meshData.size();

  // Create the vertex buffer.
  {
    ThrowIfFailed(m_device->CreateCommittedResource(&CD3DX12_HEAP_PROPERTIES(D3D12_HEAP_TYPE_DEFAULT), D3D12_HEAP_FLAG_NONE,
                                                    &CD3DX12_RESOURCE_DESC::Buffer(SampleAssets::VertexDataSize), D3D12_RESOURCE_STATE_COPY_DEST,
    ...
    ThrowIfFailed(m_device->CreateCommittedResource(&CD3DX12_HEAP_PROPERTIES(D3D12_HEAP_TYPE_UPLOAD), D3D12_HEAP_FLAG_NONE,
                                                    &CD3DX12_RESOURCE_DESC::Buffer(SampleAssets::VertexDataSize), D3D12_RESOURCE_STATE_GENERIC_READ,
                                                    nullptr, IID_PPV_ARGS(&vertexBufferUploadHeap)));
  ...
  {
    ThrowIfFailed(m_device->CreateCommittedResource(&CD3DX12_HEAP_PROPERTIES(D3D12_HEAP_TYPE_DEFAULT), D3D12_HEAP_FLAG_NONE,
                                                    &CD3DX12_RESOURCE_DESC::Buffer(SampleAssets::IndexDataSize), D3D12_RESOURCE_STATE_COPY_DEST,
                                                    nullptr, IID_PPV_ARGS(&m_indexBuffer)));

    ThrowIfFailed(m_device->CreateCommittedResource(&CD3DX12_HEAP_PROPERTIES(D3D12_HEAP_TYPE_UPLOAD), D3D12_HEAP_FLAG_NONE,
                                                    &CD3DX12_RESOURCE_DESC::Buffer(SampleAssets::IndexDataSize), D3D12_RESOURCE_STATE_GENERIC_READ,
    ...
    m_commandList->ResourceBarrier(
        1, &CD3DX12_RESOURCE_BARRIER::Transition(m_indexBuffer.Get(), D3D12_RESOURCE_STATE_COPY_DEST, D3D12_RESOURCE_STATE_INDEX_BUFFER));
  ...
  // Create the textures and sampler.
  {
    // Procedurally generate an array of textures to use as city materials.
    {
      ...
      for (UINT i = 0; i < CityMaterialCount; ++i) {
        ThrowIfFailed(m_device->CreateCommittedResource(&CD3DX12_HEAP_PROPERTIES(D3D12_HEAP_TYPE_DEFAULT), D3D12_HEAP_FLAG_NONE, &textureDesc,
                                                        D3D12_RESOURCE_STATE_COPY_DEST, nullptr, IID_PPV_ARGS(&m_cityMaterialTextures[i])));
      ...
      // Upload texture data to the default heap resources.
      {
        ...
        const UINT64 uploadBufferStep =
            GetRequiredIntermediateSize(m_cityMaterialTextures[0].Get(), 0, subresourceCount);  // All of our textures are the same size in this case.
        ...
        for (int i = 0; i < CityMaterialCount; ++i) {
          ...
          UpdateSubresources(m_commandList.Get(), m_cityMaterialTextures[i].Get(), materialsUploadHeap.Get(), i * uploadBufferStep, 0,
                             subresourceCount, &textureData);
          m_commandList->ResourceBarrier(1, &CD3DX12_RESOURCE_BARRIER::Transition(m_cityMaterialTextures[i].Get(), D3D12_RESOURCE_STATE_COPY_DEST,
    ...
    // array in the pixel shader.
    {
      ...
      ThrowIfFailed(m_device->CreateCommittedResource(&CD3DX12_HEAP_PROPERTIES(D3D12_HEAP_TYPE_DEFAULT), D3D12_HEAP_FLAG_NONE, &textureDesc,
                                                      D3D12_RESOURCE_STATE_COPY_DEST, nullptr, IID_PPV_ARGS(&m_cityDiffuseTexture)));
      ...
      m_commandList->ResourceBarrier(1, &CD3DX12_RESOURCE_BARRIER::Transition(m_cityDiffuseTexture.Get(), D3D12_RESOURCE_STATE_COPY_DEST,
                                                                              D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE));
    }
    ...
    ThrowIfFailed(m_device->CreateCommittedResource(
        ...
        &CD3DX12_RESOURCE_DESC::Tex2D(
            ...
            D3D12_RESOURCE_FLAG_ALLOW_DEPTH_STENCIL |
                D3D12_RESOURCE_FLAG_DENY_SHADER_RESOURCE),  // Performance tip: Deny shader resource access to resources that don't need shader resource views.
...
void DynamicIndexing::OnDestroy() {
...
void DynamicIndexing::OnKeyDown(UINT8 key) {
  m_camera.OnKeyDown(key);
}

void DynamicIndexing::OnKeyUp(UINT8 key) {
  m_camera.OnKeyUp(key);
}

// Create the resources that will be used every frame.
void DynamicIndexing::CreateFrameResources() {
  // Initialize each frame resource.
  CD3DX12_CPU_DESCRIPTOR_HANDLE cbvSrvHandle(m_cbvSrvHeap->GetCPUDescriptorHandleForHeapStart(), CityMaterialCount + 1,
                                             m_cbvSrvDescriptorSize);  // Move past the SRVs.
  for (UINT i = 0; i < FrameCount; i++) {
    ...
    pFrameResource->InitBundle(m_device.Get(), m_pipelineState.Get(), i, m_numIndices, &m_indexBufferView, &m_vertexBufferView, m_cbvSrvHeap.Get(),
                               m_cbvSrvDescriptorSize, m_samplerHeap.Get(), m_rootSignature.Get());

    m_frameResources.push_back(pFrameResource);
  }
}

void DynamicIndexing::PopulateCommandList(FrameResource* pFrameResource) {
  ...
  if (UseBundles) {
    ...
    pFrameResource->PopulateCommandList(m_commandList.Get(), m_currentFrameResourceIndex, m_numIndices, &m_indexBufferView, &m_vertexBufferView,
                                        m_cbvSrvHeap.Get(), m_cbvSrvDescriptorSize, m_samplerHeap.Get(), m_rootSignature.Get());
  }
