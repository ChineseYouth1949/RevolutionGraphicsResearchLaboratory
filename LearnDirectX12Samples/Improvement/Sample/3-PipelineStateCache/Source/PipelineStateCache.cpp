...
const float PipelineStateCache::IntermediateClearColor[4] = {0.0f, 0.2f, 0.3f, 1.0f};

PipelineStateCache::PipelineStateCache(UINT width, UINT height, std::wstring name)
...
void PipelineStateCache::OnInit() {
  UpdateWindowTextPso();
...
void PipelineStateCache::LoadPipeline() {
...
void PipelineStateCache::LoadAssets() {
  ...
  // Vertex and Index Buffer.
  {
    ...
    ThrowIfFailed(m_device->CreateCommittedResource(&CD3DX12_HEAP_PROPERTIES(D3D12_HEAP_TYPE_DEFAULT), D3D12_HEAP_FLAG_NONE,
                                                    &CD3DX12_RESOURCE_DESC::Buffer(vertexIndexBufferSize), D3D12_RESOURCE_STATE_COPY_DEST, nullptr,
    ...
    ThrowIfFailed(m_device->CreateCommittedResource(&CD3DX12_HEAP_PROPERTIES(D3D12_HEAP_TYPE_UPLOAD), D3D12_HEAP_FLAG_NONE,
                                                    &CD3DX12_RESOURCE_DESC::Buffer(vertexIndexBufferSize), D3D12_RESOURCE_STATE_GENERIC_READ, nullptr,
    ...
    m_commandList->ResourceBarrier(
        1, &CD3DX12_RESOURCE_BARRIER::Transition(m_vertexIndexBuffer.Get(), D3D12_RESOURCE_STATE_COPY_DEST,
...
void PipelineStateCache::OnUpdate() {
...
// Render the scene.
void PipelineStateCache::OnRender() {
  try {
...
void PipelineStateCache::ReleaseD3DResources() {
  m_fence.Reset();
...
void PipelineStateCache::RestoreD3DResources() {
...
void PipelineStateCache::OnDestroy() {
...
void PipelineStateCache::OnKeyDown(UINT8 key) {
  m_camera.OnKeyDown(key);
}

void PipelineStateCache::OnKeyUp(UINT8 key) {
...
void PipelineStateCache::PopulateCommandList() {
  ...
  PIXBeginEvent(m_commandList.Get(), 0, L"Post-processing");
  {
    ...
    for (UINT i = PostBlit; i < EffectPipelineTypeCount; i++) {
      if (m_enabledEffects[i]) {
        CD3DX12_VIEWPORT viewport((quadCount % quadsX) * (m_viewport.Width / quadsX), (quadCount / quadsY) * (m_viewport.Height / quadsY),
                                  m_viewport.Width / quadsX, m_viewport.Height / quadsY);
...
void PipelineStateCache::ToggleEffect(EffectPipelineType type) {
...
void PipelineStateCache::UpdateWindowTextPso() {
...
void PipelineStateCache::WaitForGpu() {
...
void PipelineStateCache::MoveToNextFrame() {
  // Schedule a Signal command in the queue.
