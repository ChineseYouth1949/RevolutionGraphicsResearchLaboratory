...
void PSOLibrary::SetPipelineState(ID3D12Device* pDevice, ID3D12RootSignature* pRootSignature, ID3D12GraphicsCommandList* pCommandList,
                                  _In_range_(0, EffectPipelineTypeCount - 1) EffectPipelineType type, UINT frameIndex) {
  ...
  if (type > BaseUberShader) {
    ...
    if (!isBuilt && m_useUberShaders) {
      ...
      if (!isInFlight) {
        ...
        m_workerThreads[type].threadHandle = CreateThread(nullptr, 0, reinterpret_cast<LPTHREAD_START_ROUTINE>(CompilePSO),
                                                          reinterpret_cast<void*>(&m_workerThreads[type]), CREATE_SUSPENDED, nullptr);
...
void PSOLibrary::CompilePSO(CompilePSOThreadData* pDataPackage) {
  ...
  if (useCache && (pLibrary->m_psoCachingMechanism == PSOCachingMechanism::PipelineLibraries)) {
    ...
    if (E_INVALIDARG == hr) {
      // A PSO with the specified name doesn抰 exist, or the input desc doesn抰 match the data in the library.
  ...
  } else if (useCache && (pLibrary->m_psoCachingMechanism == PSOCachingMechanism::CachedBlobs)) {
